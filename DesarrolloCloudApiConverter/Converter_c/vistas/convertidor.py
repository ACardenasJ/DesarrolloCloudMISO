...
def set_file_bkt(file_name, id_task):
    url_bucket = 'http://{}/api/BucketPo/{}/{}'.format(BUCKET_URL,file_name,id_task)
    file_path = os.path.join(PROCESS_DIRECTORY, file_name)
    files = {'file': open(file_path, 'rb')}
    requests.post(url_bucket, files=files)

def fire_and_forget(file_name, id_task):
    threading.Thread(target=set_file_bkt, args=(file_name, id_task)).start()

def subir_archivo(file_name, id_task):
    fire_and_forget(file_name, id_task)
    print("subiendo archivo", flush=True)
...
class converter():
    ...
    #MP3 to WAV
    def mp3_wav(self,file_name, id_task):
        ...
        subir_archivo(file_name+".wav", id_task)
        #guardar archivo en bucket
    #MP3 to WMA
    def mp3_WMA(self,file_name, id_task):
        descargador(file_name+".mp3") 
        ...
        print(output,flush=True)
        print(error,flush=True)
        subir_archivo(file_name+".wma", id_task)
    #WAV to MP3
    def wav_mp3(self,file_name, id_task):
        descargador(file_name+".wav")
        ...
        output, error = process.communicate()
        subir_archivo(file_name+".mp3", id_task)

    #WAV to WMA
    def wav_wma(self,file_name, id_task):
        descargador(file_name+".wav")
    ...
    #WMA to MP3
    def wma_mp3(self,file_name, id_task):
        descargador(file_name+".wma")
    ...
    #WMA to WAV
    def WMA_WAV(self,file_name, id_task):
        descargador(file_name+".wma")
